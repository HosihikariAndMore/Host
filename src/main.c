...
int __libc_start_main(int (*main)(int, char **, char **), int argc, char **ubp_av, void (*init)(), void (*fini)(),
...
int load_hostfxr()
{
    ...
    assert(init_fptr != NULL && "Failure: hostfxr_initialize_for_runtime_config()");
    get_delegate_fptr = (hostfxr_get_runtime_delegate_fn)dlsym(lib, "hostfxr_get_runtime_delegate");
    assert(get_delegate_fptr != NULL && "Failure: hostfxr_get_runtime_delegate()");
